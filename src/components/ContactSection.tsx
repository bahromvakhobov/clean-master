...
const ContactSection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/mreyoekr", {
        method: "POST",
        headers: {
  ...
  const inputClass =
    "w-full px-5 py-3.5 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground/60 focus:ring-2 focus:ring-primary/20 focus:border-primary/40 outline-none transition-all duration-300 text-[15px]";
        ...
        <AnimatedSection>
          <div className="text-center max-w-2xl mx-auto mb-20">
            ...
            <p className="section-subheading">
              Savollaringiz bormi? Biz doimo aloqadamiz
            </p>
          </div>
        </AnimatedSection>
          ...
          <AnimatedSection delay={0.12} className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="card-premium p-8 space-y-5">
              ...
              <button
                ...
                className="group w-full gradient-primary text-primary-foreground py-4 rounded-xl font-bold hover:shadow-[0_8px_30px_-6px_hsl(214_90%_52%/0.4)] transition-all duration-300 flex items-center justify-center gap-2 text-[15px] disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {submitState === "loading" ? "Yuborilmoqda..." : "Yuborish"}
                {submitState !== "loading" && (
                  <ArrowRight
                    size={16}
